...
import org.jenkins.plugins.yc.YCComputer;
import org.jenkins.plugins.yc.YandexCloud;
import org.jenkins.plugins.yc.YandexTemplate;
...
public class MinimumInstanceChecker {
    ...
    private static Stream<Computer> agentsForTemplate(@Nonnull YandexTemplate agentTemplate) {
        return (Stream<Computer>) Arrays.stream(Jenkins.get().getComputers())
                .filter(computer -> computer instanceof YCComputer)
                .filter(computer -> {
                    YandexTemplate computerTemplate = ((YCComputer) computer).getSlaveTemplate();
                    return computerTemplate != null
                            && Objects.equals(computerTemplate.description, agentTemplate.description);
                });
    }

    public static int countCurrentNumberOfAgents(@Nonnull YandexTemplate agentTemplate) {
        return (int) agentsForTemplate(agentTemplate).count();
    }

    public static int countCurrentNumberOfSpareAgents(@Nonnull YandexTemplate agentTemplate) {
    ...
    public static int countCurrentNumberOfProvisioningAgents(@Nonnull YandexTemplate agentTemplate) {
        return (int) agentsForTemplate(agentTemplate)
                .filter(computer -> computer.countBusy() == 0)
    ...
    */
    public static int countQueueItemsForAgentTemplate(@Nonnull YandexTemplate agentTemplate) {
        return (int)
                Queue
                        .getInstance()
                        .getBuildableItems()
    ...
    public static void checkForMinimumInstances() {
        Jenkins.get().clouds.stream()
                .filter(cloud -> cloud instanceof YandexCloud)
                .map(cloud -> (YandexCloud) cloud)
                .forEach(cloud -> {
